...
from pointpd.engines.defaults import (
    default_argument_parser,
    default_config_parser,
    default_setup,
    create_ddp_model,
)
from pointpd.engines.launch import launch
from pointpd.models import build_model
from pointpd.datasets import build_dataset
from pointpd.utils.logger import get_root_logger
from pointpd.engines.test import TEST
...
def main_worker(cfg):
    ...
    logger = get_root_logger(
        log_file=os.path.join(cfg.save_path, "test-{}.log".format(weight_name))
    )
    ...
    model = create_ddp_model(
        model.cuda(), broadcast_buffers=False, find_unused_parameters=False
    )
    ...
    if os.path.isfile(cfg.weight):
        ...
        logger.info(
            "=> Loaded weight '{}' (epoch {})".format(cfg.weight, checkpoint["epoch"])
        )
